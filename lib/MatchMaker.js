...
class MatchMaker {
    ...
    async connectToRoom(client, roomId) {
     ...
     */
    async onJoinRoomRequest(client, roomToJoin, clientOptions) {
    ...
    async remoteRoomCall(roomId, method, args, rejectionTimeout = PRESENCE_TIMEOUT) {
        const room = this.localRooms[roomId];
        if (!room) {
    ...
    async joinById(roomId, clientOptions, isReconnect) {
    ...
    async getAvailableRoomByScore(roomName, clientOptions) {
        return (await this.getRoomsWithScore(roomName, clientOptions)).
            sort((a, b) => b.score - a.score);
    }
    async create(roomName, clientOptions) {
    ...
    async getAvailableRooms(roomName, roomMethodName = 'getAvailableData') {
        const roomIds = await this.presence.smembers(roomName);
        const availableRooms = [];
        await Promise.all(roomIds.map(async (roomId) => {
            const availability = await this.remoteRoomCall(roomId, roomMethodName);
            if (availability) {
    ...
    async getAllRooms(roomName, roomMethodName = 'getAvailableData') {
    ...
    async cleanupStaleRooms(roomName) {
        //
    ...
    async getRoomsWithScore(roomName, clientOptions) {
        ...
        await Promise.all(roomIds.map(async (roomId) => {
            ...
            if (!localRoom) {
                ...
                roomsWithScore.push({
                    roomId,
                    score: (await localRoom.requestJoin(clientOptions, false)),
                });
            }
            return true;
        }));
