    ...
    constructor(options = {}) {
        this.verifyClient = (info, next) => __awaiter(this, void 0, void 0, function* () {
            ...
            if (req.roomId) {
                ...
                this.matchMaker
                    .remoteRoomCall(req.roomId, "onAuth", [req.options])
                    .then(result => {
                    if (!result) {
                        return next(false);
                    }
        ...
        this.onConnection = (client, req) => {
            ...
            client.on("error", err => {
                Debug_1.debugError(err.message + "\n" + err.stack);
            });
            const roomId = upgradeReq.roomId;
            if (roomId) {
                this.matchMaker.connectToRoom(client, upgradeReq.roomId).catch(e => {
    ...
    onMessageMatchMaking(client, message) {
        ...
        if (message[0] === Protocol_1.Protocol.JOIN_ROOM) {
            ...
            if (!this.matchMaker.hasHandler(roomName) && !index_1.isValidId(roomName)) {
                ...
                this.matchMaker
                    .onJoinRoomRequest(client, roomName, joinOptions)
        ...
        else if (message[0] === Protocol_1.Protocol.PING) {
            // keep-alive ping.
            return;
        }
        else {
